...
/**
 * \file    invoicingorientedorders/admin/setup.php
 * \ingroup invoicingorientedorders
 * \brief   Invoicingorientedorders setup page.
 */
...
require_once '../lib/invoicingorientedorders.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "invoicingorientedorders@invoicingorientedorders"));

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(array('invoicingorientedorderssetup', 'globalsetup'));
...
// Setup conf INVOICINGORIENTEDORDERS_MYPARAM2 as a simple textarea input but we replace the text of field title
...
// Setup conf INVOICINGORIENTEDORDERS_MYPARAM6
$formSetup->newItem('INVOICINGORIENTEDORDERS_MYPARAM6')->setAsSecureKey()->enabled = 0; // disabled

// Setup conf INVOICINGORIENTEDORDERS_MYPARAM7
$formSetup->newItem('INVOICINGORIENTEDORDERS_MYPARAM7')->setAsProduct();

$formSetup->newItem('Title')->setAsTitle();

// Setup conf INVOICINGORIENTEDORDERS_MYPARAM8
$item = $formSetup->newItem('INVOICINGORIENTEDORDERS_MYPARAM8');
$TField = array(
...
$item->helpText = $langs->transnoentities('INVOICINGORIENTEDORDERS_MYPARAM8');


// Setup conf INVOICINGORIENTEDORDERS_MYPARAM9
$formSetup->newItem('INVOICINGORIENTEDORDERS_MYPARAM9')->setAsSelect($TField);


// Setup conf INVOICINGORIENTEDORDERS_MYPARAM10
$item = $formSetup->newItem('INVOICINGORIENTEDORDERS_MYPARAM10');
$item->setAsColor();
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/invoicingorientedorders/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=invoicingorientedorders-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'INVOICINGORIENTEDORDERS_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'INVOICINGORIENTEDORDERS_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$page_name = "InvoicingorientedordersSetup";
...
$head = invoicingorientedordersAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "invoicingorientedorders@invoicingorientedorders");
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'INVOICINGORIENTEDORDERS_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'INVOICINGORIENTEDORDERS_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
